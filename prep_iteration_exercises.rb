...
class Array
  ...
  def bubble_sort(&prc)
    ...
    while !sorted
      sorted = true
      (0...slef.length - 1).each do |i|
        if slef[i] > slef[i+1]
          slef[i], slef[i+1] = slef[i+1], slef[i]
          sorted = false
        end
      end
    end
...
def subwords(word, dictionary)
  subs = []
  start_idx = 0
  while start_idx < word.length
    (start_idx...word.length).each do |end_idx|
      subword = word[start_idx..end_idx]
      subs <<  subword if dictionary.include?(subword) && !subs.include?(subword)
    end
    start_idx += 1
  end
  subs
...
def concatenate(strings)
  strings.my_inject {|acc, str| acc + str}
end